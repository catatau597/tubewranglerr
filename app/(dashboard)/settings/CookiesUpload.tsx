      ...
      return (
        <div className="flex flex-col gap-2 max-w-md">
          ...
          <div className="flex gap-2 items-center">
            <label className="bg-gray-200 hover:bg-gray-300 text-gray-800 rounded px-3 py-1 text-sm cursor-pointer">
              Escolher arquivos
              <input
                type="file"
                accept=".txt"
            ...
            <button className="bg-blue-600 text-white rounded px-3 py-1 text-sm" onClick={async () => { await handleUpload(); fetchCookieFiles(); }} disabled={!files || files.length === 0 || loading}>Enviar</button>
          </div>
            ...
            <ul className="text-xs mt-1">
              ...
              {cookieFiles.map(f => (
                <li key={f.name} className="flex items-center gap-2">
                  <span>{f.name}</span>
                  <button className="bg-red-600 text-white rounded px-2 py-0.5 text-xs" onClick={async () => { await handleDelete(f.name); fetchCookieFiles(); }} disabled={loading}>Excluir</button>
                </li>
              ))}
            </ul>
          </div>
